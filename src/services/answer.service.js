    ...
    return prisma.$transaction(async (transaction) => {
        ...
        if (existingAnswer.question.type === 'ESSAY') {
            ...
            if (
                ...
                hasManualGradeFields
            ) {
                throw createError(
                    'Não é permitido editar a resposta e realizar a correção manual na mesma requisição.',
                    400,
                );
            }

            if (hasManualGradeFields) {
                if (
                    existingAnswer.submission.status !== 'SUBMITTED' &&
                    existingAnswer.submission.status !== 'CORRECTED'
                ) {
                    throw createError(
                        'A correção manual só pode ser realizada após o envio da submissão.',
                        409,
                    );
                }
                ...
                if (
                    ...
                    throw createError(
                        'O campo score é obrigatório na primeira correção da resposta dissertativa.',
                        400,
                    );
                }
                ...
                if (data.score !== undefined) {
                    if (
                        ...
                        throw createError(
                            'O campo score deve ser um número válido.',
                            400,
                        );
                    }
                    ...
                    const maximumScore = Number(
                        existingAnswer.question.points,
                    );

                    if (score < 0) {
                        throw createError(
                            'O campo score não pode ser negativo.',
                            400,
                        );
                    }
                    ...
                    if (score > maximumScore) {
                        throw createError(
                            `O campo score não pode ultrapassar ${maximumScore} ponto(s).`,
                            400,
                        );
                    }
                    ...
                    if (data.isCorrect === undefined) {
                        updateData.isCorrect = score === maximumScore;
                    }
                }
                ...
                if (data.feedback !== undefined) {
                    if (
                        ...
                        throw createError(
                            'O campo feedback deve ser um texto ou null.',
                            400,
                        );
                    }
                ...
                if (data.isCorrect !== undefined) {
                    if (
                        ...
                        throw createError(
                            'O campo isCorrect deve ser booleano ou null.',
                            400,
                        );
                    }
